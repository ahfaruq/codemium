...
for folder,phrase in expected.items(): assert phrase in (root/f'plugins/codemium/skills/{folder}/SKILL.md').read_text(), phrase
readme=(root/'README.md').read_text()
assert '@cm' in readme and '$codemium:' not in readme
assert 'FAST | `low`' in readme and 'CRITICAL | `xhigh`' in readme
assert '## Numbers' in readme and 'SYNTHETIC' in (root/'benchmarks/demo-numbers.svg').read_text()
...
print('PASS: manifests, @cm, reasoning policy, Numbers dashboard, and core doctrine')
PYEOF
...
printf '%s\n' 'PASS: benchmark renderer + synthetic publication gate'
printf '%s\n' 'ALL CHECKS PASSED'
