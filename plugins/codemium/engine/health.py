...
from parsers import parser_runtime
...
def manifest_worktree_fresh(root: Path, manifest: dict) -> tuple[bool, list[str]]:
    ...
    changed: set[str] = set(); manifest_paths = set(files)
    for rel, meta in files.items():
        p = root / rel
        try: current = sha256_bytes(p.read_bytes())
        except OSError:
            changed.add(rel); continue
        if current != meta.get("sha256"): changed.add(rel)
    try: current_paths = set(discover_files(root))
    except OSError: current_paths = manifest_paths
    changed.update(current_paths - manifest_paths)
    rows = sorted(changed); return not rows, rows[:50]


def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("--root", default="."); ns = ap.parse_args()
    root = Path(ns.root).resolve(); s = state_root(root)
    graph = read_json(s / "repository/graph.json", {}); manifest = read_json(s / "repository/manifest.json", {}); head = git(root, "rev-parse", "HEAD")
    ...
    provenance = Counter(e.get("provenance", "UNKNOWN") for e in graph.get("edges", [])); parsers = Counter(f.get("parser", "unknown") for f in graph.get("files", [])); capabilities = Counter(); languages = Counter(f.get("language", "unknown") for f in graph.get("files", []))
    for f in graph.get("files", []):
        for cap in f.get("capabilities", []): capabilities[cap] += 1
    cross_edges = [e for e in graph.get("edges", []) if e.get("cross_language")]
    out = {
        ...
        "repository_graph": {
            ...
            "parsers": dict(sorted(parsers.items())), "languages": dict(sorted(languages.items())), "capability_coverage": dict(sorted(capabilities.items())),
            "edge_provenance": dict(sorted(provenance.items())), "cross_language_edges": len(cross_edges),
            "cross_language_relations": dict(sorted(Counter(e.get("relation", "UNKNOWN") for e in cross_edges).items())),
            "parser_runtime": parser_runtime(), "incremental": graph.get("incremental", {}),
        },
